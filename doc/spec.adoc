...
** `release` - the latest version marked as `release` in the Maven repositories
...
* the fallback strategy is `latest` or `release` but underlying Maven repository contains no artifacts with required `groupId` and `artifactId`
