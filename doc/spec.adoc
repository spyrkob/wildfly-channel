...
* An optional `blocklist` that defines Maven coordinates of blocklist artifact. This field is composed of
** A required `groupId` that corresponds to Maven GroupId of the blocklist artifact.
** A required `artifactId` that corresponds to Maven ArtifactId of the blocklist artifact.
** A required `versionPattern` that corresponds to a Pattern through which the available blocklist artifacts are matched.
...
### Version 1.0.1
* Adding `blocklist` element to the Channel
