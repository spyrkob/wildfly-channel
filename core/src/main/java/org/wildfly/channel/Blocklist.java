...
public class Blocklist {
   ...
   private static final String SCHEMA_1_0_0_FILE = "org/wildfly/channel/blocklist/v1.0.0/schema.json";
   ...
   public Blocklist(@JsonProperty(value = "schemaVersion", required = true) String schemaVersion,
                    @JsonProperty(value = "exclusions") Set<BlocklistEntry> entries) {
   ...
   public static Blocklist from(URL blocklistUrl) {
      ...
      try {
         ...
         if (!messages.isEmpty()) {
            throw new InvalidChannelException("Invalid blocklist", messages);
         }
